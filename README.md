...
gantt
    ...
    로그인/회원가입/비번찾기 페이지 백연결 + 토큰 : crit, 2021-05-05, 3d
    마이페이지 : done, 2021-04-26, 5d
    마이페이지 백연결 : crit, 2021-05-07, 1d
    정보공유 게시판 : done, 2021-04-26, 4d
    Qna 게시판 : done, 2021-05-02, 8d
    코드공유 게시판 : done, 2021-04-30, 8d
    게시판 백연결 : active, 2021-05-08, 2d
    플래너 + 백연결 : crit, 2021-05-07, 5d
    명예의 전당 : done, 2021-05-07, 1d
    
    section Backend
    배포하기 : crit, 2021-05-07, 2d
...
* **방(학교)생성 및 리스트**
  ...
  >
  > b. 현재 활동중인 사람이 있어서 활성화된 방은 Online, 비활성화시 Offline표시가 된다.  
  >
  > c. 각 방(학교)에는 학교 이름, 썸네일(대표 이미지), 제한인원, 학교 설명(규칙)이 표시된다.
  >
  > d. 방(학교) 리스트는 최근 만들어진 순, 인기순(접속자가 많은 순)으로 확인할 수 있다.
  >
  > e. 방(학교) 이름, 설명에 포함된 키워드를 검색할 수 있다.

* **게임 맵(유니티)**  
  > a. 방(학교)에 입작하면 전체 게임 맵이 보이고 본인의 캐릭터를 이동할 수 있다.
  >
  > b. 현재 접속중인 학생들의 활동 상태를 "온라인/자리비움/방해금지"로 확인할 수 있다. 
  >
  > c. 현재 접속 중인 학생들과 실시간으로 채팅할 수 있다. 
  >
  > d. 방의 컨셉에 따라 상황에 맞는 NPC가 배치되고 이를 이용해 튜토리얼이 진행된다.  
...
* **화상미팅 기능**
  ...
  >
  > c. 현재 접속중인 학생들의 활동 상태를 "온라인/자리비움/방해금지"으로 확인할 수 있다.  
  >
...
* **웹(학교 공간) 기능**
  > a. 컴퓨터 교실에서는 웹캠, 오디오, 웹채팅을 지원하며 정보공유/코드공유 게시판에서 마크다운 언어를 사용한다.
  >
  > b. 정보공유 게시판에서는 학습하며 정리한 내용을 학생들과 함께 공유할 수 있다.  
  >
  > c. 코드공유 게시판에서는 알고리즘 문제를 풀면서 풀리지 않았던 문제를 공유할 수 있고, 코드 컴파일러를 실행해 제출한 코드를 컴파일하고 성공 여부, 실행 시간, 메모리 등을 확인할 수 있다.
  >
  > d. 코드를 하나 제출하면 50p가 적립되며 100p단위로 레벨이 올라간다. 로비의 명예의 전당에서 상위 랭킹 10명의 학생을 확인할 수 있다.   
  >
  > e. 도서관에서는 플래너 게시판이 마크다운 언어로 제공되고, 공부 쓴소리 영상(유튜브), 백색소음 선택 기능, 스톱워치 기능 등이 제공된다.  
  >
  > f. 급식소에서는 웹캠, 화면공유, 웹채팅을 지원하며 추천 영상(유튜브 인기 영상)을 제공한다.  
  > 
  > g. 운동장/광장에서는 학생들과 함께 즐길 수 있는 게임이 제공된다.  
  >
  > h. 휴게실에서는 웹캠, 오디오, 웹채팅을 지원한다.  
  > 
  > i. 방명록에서는 학생들이 로그인한 기록을 확인할 수 있다.  
