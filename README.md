...
* **방(학교)생성 및 리스트**
  ...
  >
  > c. 방(학교) 리스트는 최근 만들어진 순, 인기순(접속자가 많은 순)으로 확인할 수 있다.
  >
  > d. 방(학교) 이름, 설명에 포함된 키워드를 검색할 수 있다.
...
* **게임 맵(유니티)**  
  ...
  >
  > c. 방의 컨셉에 따라 상황에 맞는 NPC가 배치되고 이를 이용해 튜토리얼이 진행된다.  
...
* **웹(학교 공간) 기능**
  ...
  >
  > d. 도서관에서는 플래너 게시판이 마크다운 언어로 제공되고, 공부 쓴소리 영상(유튜브), 백색소음 선택 기능, 스톱워치 기능 등이 제공된다.  
  >
  > e. 급식소에서는 웹캠, 화면공유, 웹채팅을 지원하며 추천 영상(유튜브 인기 영상)을 제공한다.  
  > 
  > f. 운동장/광장에서는 학생들과 함께 즐길 수 있는 게임이 제공된다.  
  >
  > g. 휴게실에서는 웹캠, 오디오, 웹채팅을 지원한다.  
  > 
  > h. 방명록에서는 학생들이 로그인한 기록을 확인할 수 있다.  
